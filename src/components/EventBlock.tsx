...
export default function EventBlock({ id, title, date, category, scope, tickers, userVote, onClick, draggable = false, isPlaced = false, onDragStart, onDragEnd }: Props) {
  ...
  return (
    <div
      ...
      onClick={() => onClick?.(id)}
      className={`text-xs px-1.5 py-1 rounded cursor-pointer hover:opacity-80 transition-opacity bg-white border border-gray-200 hover:border-gray-300 ${draggable ? 'cursor-move' : ''}`}
    >
